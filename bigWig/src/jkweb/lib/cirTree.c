...
slReverse(&list);
verbose(2, "Made %d primary index nodes out of %"PRIuMAX" items\n", slCount(list), itemCount);
...
for (i=0; i<levelCount; ++i)
    ...
    verbose(2, "level %d: size %d, offset %"PRIuMAX"\n", i, levelSizes[i], levelOffsets[i]);
    }
...
for (i=0; i<=finalLevel; ++i)
    ...
    if (ftell(f) != levelOffsets[i+1])
        errAbort("Internal error: offset mismatch (%"PRIuMAX" vs %"PRIuMAX") line %d of %s\n", (bits64)ftell(f), levelOffsets[i+1], __LINE__, __FILE__);
    }
...
verbose(3, "rFindOverlappingBlocks %"PRIuMAX" %u:%u-%u.  childCount %d. isLeaf %d\n", indexFileOffset, chromIx, start, end, (int)childCount, (int)isLeaf);
...
verbose(3, "rEnumerateBlocks %"PRIuMAX" childCount %d. isLeaf %d\n", indexFileOffset, (int)childCount, (int)isLeaf);
