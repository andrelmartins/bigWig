...
#include <sys/wait.h>
#else
int pipe(int pipefd[2]); // defined in osmingw.c
#undef BYTE
#undef WORD
#undef UWORD
#undef boolean
...
/* A single process in a pipeline */
{
    ...
    char * fullCmd;        /* space separated full command line, for use with CreateProcess */
    HANDLE * pHandle;      /* Windows Process handle */
    struct threadMem * data; /* optional data storage pointer when creating a thread */
...
for (i = 0; proc->cmd[i] != NULL; i++)
    freeMem(proc->cmd[i]);
freeMem(proc->cmd);
freeMem(proc->fullCmd);
if (proc->pHandle != NULL) {
    CloseHandle(proc->pHandle);
}
if (proc->data != NULL) {
    HeapFree(GetProcessHeap(), 0, proc->data);
}
freeMem(proc);
}
...
static void plProcSetup(struct plProc* proc, int stdinFd, int stdoutFd, int stderrFd, STARTUPINFO * StartUpSearch)
...
#ifdef __WIN32__
if (proc->pl->options & pipelineSigpipe)
...
if (signal(SIGPIPE, ((proc->pl->options & pipelineSigpipe) ? SIG_DFL : SIG_IGN)) == SIG_ERR)
    ...
    if (stdinFd != STDIN_FILENO) {
        StartUpSearch->hStdInput = _get_osfhandle(stdinFd);
    } else {
        StartUpSearch->hStdInput = GetStdHandle (STD_INPUT_HANDLE);
    }

    if (stdoutFd != STDOUT_FILENO) {
        StartUpSearch->hStdOutput = _get_osfhandle(stdoutFd);
    } else {
        StartUpSearch->hStdOutput = GetStdHandle (STD_OUTPUT_HANDLE);
    }

    if (stderrFd != STDERR_FILENO) {
        StartUpSearch->hStdError = _get_osfhandle(stderrFd);  
    } else {
        StartUpSearch->hStdError = GetStdHandle (STD_ERROR_HANDLE);
    }
}
...
/* child part of process startup. */
{
    ...
    /* Prepare redirects */
    plProcSetup(proc, stdinFd, stdoutFd, stderrFd, &StartUpSearch);

    /* Create a process to execute the command line. */
    if (CreateProcess (NULL, proc->fullCmd, NULL, NULL,
            TRUE, 0, NULL, NULL, &StartUpSearch, &ProcessInfo) == FALSE) {
        errnoAbort("CreateProcess failed: %s", proc->cmd[0]);
    }
    ...
    proc->pHandle = ProcessInfo.hProcess;
}
...
static DWORD WINAPI MemWriteThreadFunction( LPVOID lpParam ) 
{
    struct threadMem * data = (struct threadMem*) lpParam;

    /* write to stdoutFd */
    ssize_t wrCnt = write(data->stdoutFd, data->otherEndBuf, data->otherEndBufSize);
    if (wrCnt < 0)
...
/* implements child "process" (thread) to write memory buffer to pipeline after
 * fork */
{
    DWORD   dwThreadId;
    struct threadMem * data = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(struct threadMem));

    proc->data = data;
    data->otherEndBuf = otherEndBuf;
    data->otherEndBufSize = otherEndBufSize;
    data->stdoutFd = stdoutFd;

    proc->pHandle = CreateThread( 
            NULL,                   // default security attributes
            0,                      // use default stack size  
            MemWriteThreadFunction, // thread function name
            data,                   // argument to thread function 
            0,                      // use default creation flags 
            &dwThreadId);           // returns the thread identifier 

    if (proc->pHandle == NULL) {
        errAbort("pipeline input thread creation failed");
    }
}
...
static int cmds_count(char *** cmds) {
    int iCmd = 0;
    for (; cmds[iCmd] != NULL; iCmd++);
    return iCmd;
}
...
if (cmds_count(cmds) > MAXIMUM_WAIT_OBJECTS) {
    errAbort("too many commands in pipeline");
}
...
/* start process (on Windows the execProcChild will create the thread/process and execute) */
...
{
    /* temporary array to hold all process handles */
    HANDLE * hArray = needMem((pl->numRunning)*sizeof(HANDLE));

    WaitForMultipleObjects (pl->numRunning, hArray, TRUE, INFINITE);
...
static void pipelineExec(struct pipeline* pl, int stdinFd, int stdoutFd, int stderrFd,
                         void *otherEndBuf, size_t otherEndBufSize)
/* On Windows we just launch all processes and ignore anything else */
{
assert(pl->groupLeader < 0);  // should not be set
pipelineGroupExec(pl, stdinFd, stdoutFd, stderrFd, otherEndBuf, otherEndBufSize);
// TODO: Create a Job Object and associate all child processes with it
} 
